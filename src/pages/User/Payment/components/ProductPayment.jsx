...
const ProductPayment = ({setTotalSum}) => {
  ...
  useEffect(() => {
    if (accessToken) {
      getCat(setData, setTotalSum, setError);
    }
  }, []);
  ...
  useEffect(() => {
    if (error) {
      if (error.status === 404) {
        navigate("/cart", { state: { errorMessage: "Корзина пуста." } });
